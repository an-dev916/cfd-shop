...
const FeatureProductsSection = ({
  ...
  const onChangTab = (slug) => {
    if (slug !== selectedCateSlug) {
      setRenderProducts([]);
    }
    setTimeout(() => onSelectedCateSlug?.(slug), 300);
  };

  useEffect(() => {
    setRenderProducts(featuredProducts);
  }, [featuredProducts]);

  return (
    <div className="container top" style={{ minHeight: "582px" }}>
          ...
          >
            {categories?.length > 0 &&
              categories.map((category, index) => {
                ...
                return (
                  <li className="nav-item" key={slug || index}>
                    <a
                      className={`nav-link ${
                        selectedCateSlug === slug ? "active" : ""
                      }`}
                      onClick={() => onChangTab(slug)}
                      // id={`top-${slug}-link`}
                      data-toggle="tab"
                      // href={`#top-${slug}-tab`}
                      // role="tab"
                      ...
                      // aria-selected={
                      //   selectedCateSlug === slug ? "true" : "false"
                      // }
                    >
                      {name || "ERROR CATE"}
                    </a>
